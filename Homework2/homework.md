...
```
**Flow 2**:That's the flow used for extract and load and transform data in bigquery using triggers , we can use the backfill to run for multiple previous months automatically :
```yaml
...
1. Within the execution for Yellow Taxi data for the year 2020 and month 12: what is the uncompressed file size (i.e. the output file yellow_tripdata_2020-12.csv of the extract task)? : 
    using **Flow 1** to run for this month 
    the answer is :  **128.3 MB**
2. What is the rendered value of the variable file when the inputs taxi is set to green, year is set to 2020, and month is set to 04 during execution?
 
    the answer is : **green_tripdata_2020-04.csv**
    as the variable is rendered it will be in this format


3. How many rows are there for the Yellow Taxi data for all CSV files in the year 2020?

    the answer is : **24,648,499** using **Flow 2** and backfill to run for the whole year

4. How many rows are there for the Green Taxi data for all CSV files in the year 2020?
   the answer is : **1,734,051** using **Flow 2** and backfill to run for the whole year

5. How many rows are there for the Yellow Taxi data for the March 2021 CSV file?

    the answer is : **1,925,152** using **Flow 1** to run for this specific date

6. How would you configure the timezone to New York in a Schedule trigger?
Airport
    the answer is : Add a timezone property set to America/New_York in the Schedule trigger configuration
    as he Schedule trigger uses the **IANA Time Zone Database format**, which is a standard for representing time zones